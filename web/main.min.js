...
return this.reset();const params={dsName,command:"tables"};if(schema&&schema!=="")
params.schema=schema;m.request({method:"GET",url:schema?"/api/command/:dsName/:schema/:command":"/api/command/:dsName/:command",headers:App.getAuthHeaders(),params}).then((response)=>{this.tables=response.data;});},view:function(vnode){if(!this.tables||!this.tables.rows.length)
return null;const{onChange,namePrefix,value=""}=vnode.attrs||{};const name=namePrefix?`${namePrefix}[table]`:"table-select";const options=[{value:"",label:"select table…"},...this.tables.rows.map(row=>({value:row[0],label:row[0]}))];return m(SelectInput,{name,value,options,title:"set a table.",onchange:(e)=>{if(onChange)onChange(e.target.value);}});}};}
...
function DataEndpointForm(){return{type:"",dsName:"",schema:"",table:"",query:"",format:"",fileObject:null,FileInput:FileInput(),SchemaInput:SchemaInput(),TableInput:TableInput(),view:function(vnode){const{endPointType="origin"}=vnode.attrs||{};if(App.dataTransferAction){App.dataTransferAction=false;this.type="query";this.dsName=QueryPage.dsName;this.schema=QueryPage.schema;this.query=QryForm.editor.getCode().trim();this.SchemaInput.getSchemas(this.dsName,this.schema);}
return m("table",[m("tr",[m("th","Type"),m("td.w-full",m(EndpointTypeInput,{value:this.type,endPointType,onChange:(sel)=>{this.format="";this.fileObject=null;if(["file",""].includes(sel)){this.dsName=this.schema="";this.SchemaInput.reset();this.TableInput.reset();}
this.table=this.query="";this.type=sel;}}))]),this.type==="file"?m("tr",[m("th","Format"),m("td",m(FileFormatInput,{value:this.format,namePrefix:endPointType,onChange:(sel)=>{if(endPointType==="origin"){this.fileObject=null;this.FileInput.reset();}
this.format=sel;}}))]):null,this.type==="file"&&endPointType==="origin"&&this.format?m("tr",[m("th","File"),m("td",m(this.FileInput,{filename:this.fileObject?this.fileObject.name:"",namePrefix:endPointType,format:this.format,onChange:(file)=>{this.fileObject=file;}}))]):null,this.type==="table"||this.type==="query"?[m("tr",[m("th","Data source"),m("td",m(DataSourceInput,{value:this.dsName,namePrefix:endPointType,onChange:(sel)=>{this.schema=this.table="";this.dsName=sel;this.SchemaInput.getSchemas(this.dsName,this.schema);this.TableInput.getTables(this.dsName,this.schema);}}))]),m("tr",[m("th","Schema"),m("td",m(this.SchemaInput,{value:this.schema,namePrefix:endPointType,dsName:this.dsName,onChange:(sel)=>{this.table="";this.schema=sel;this.TableInput.getTables(this.dsName,this.schema);}}))]),]:null,this.type==="table"?[m("tr",[m("th","Table"),m("td",m(this.TableInput,{value:this.table,namePrefix:endPointType,dsName:this.dsName,schema:this.schema,onChange:(sel)=>{this.table=sel;}}))])]:null,this.type==="query"?[m("tr",[m("th","SQL Query"),m("td",{style:"padding-right: 0"},m('textarea',{value:this.query,name:endPointType+"[query]",onChange:(value)=>{this.query=value;}}))])]:null,]);}};}
...
m.request({method:"POST",url,params,headers:App.getAuthHeaders(),body:formData,}).then((response)=>{QryForm.executing=false
...
App.pageState.set("copy")}},"set as origin"),),),]]}}
