...
const getRequestHeaders=(body)=>{const headers={'X-CSRF-Token':document.querySelector('meta[name="csrf-token"]').getAttribute('content'),};if(!(body instanceof FormData)){headers['Content-Type']='application/json';}
return headers;}
const getRequestExtract=()=>{return function(xhr){const token=xhr.getResponseHeader("Authorization");if(token&&token.startsWith("Bearer ")){sessionStorage.setItem("token",token.split(" ")[1]);}
return JSON.parse(xhr.responseText);}}
const LogOut={view:()=>{return[m("button[type=button].mb-5",{title:"logout "+username,onclick:function(){m.request({method:"GET",url:"/logout",credentials:"include",}).catch(function(e){if(e.code===401){console.log("User logged out.");m.route.set("/");window.location.reload();}})}},"logout"),]}}
const DownloadIcon={view:({attrs})=>{const{size=18,color="currentColor",className="",style="",...rest}=attrs;const spanStyle=`vertical-align: middle;${style ? " " + style : ""}`;const svg=`
    ...
    `;return m("span",{rest,style:spanStyle},m.trust(svg));}};const App={conn:"",schema:"",theme:"",tabState:new TabState("result"),oninit:()=>{var theme=localStorage.getItem("theme");App.theme=theme?theme:window.matchMedia("(prefers-color-scheme: dark)").matches?"dark-mode":"light-mode";},view:()=>{return m("div.grid-main",{class:App.theme},m("header.area-main-header",m("nav.brand",m("a",{href:"/",title:"refresh page"},versionInfo.appName),m("span.ml-10","v",versionInfo.server),)),m("section.area-main-menu",m("div.grid",{style:"grid-template-columns: auto 1fr auto;"},m("div.grid-col",m("div",m(ConnForm)),m("div.ml-10",m(SchemaForm))),m("div.grid-col.align-items-end.ml-10",m(ConnInfos)),m("div.grid-col.ml-auto.mr-0",m("div.ml-10",m(ThemeSwitch)),m("div.ml-10",m(LogOut))),),),m("section.area-main-content",!App.conn?null:[m("div.grid-query",{oncreate:function(vnode){var h0=document.querySelector('.grid-query').offsetHeight,h1=195,h2=335,h1=Math.max(h0-h2,h1);const LayoutGrid=GridResize('.grid-query','.area-query-splitter','.area-query-editor',`${h1}px 3px auto auto`,195,false);LayoutGrid.init();}},m("section.area-query-editor",m("div.grid-q-editor-datadict",{oncreate:function(vnode){const LayoutGrid=GridResize('.grid-q-editor-datadict','.area-q-splitter','.area-q-editor',`1fr 3px 1fr`,540,true);LayoutGrid.init();}},m("section.area-q-editor",m(QryForm)),m("div.area-q-splitter.splitter.splitter-vertical"),m("section.area-q-datadict",m(DataDict)),)),m("div.area-query-splitter.splitter.splitter-horizontal"),m("div",m("section.area-query-output-menu",m("div.grid",{style:"grid-template-columns: auto auto 1fr;"},m("div.grid-col.tab.tab-b",{class:App.tabState.selectedClass("result"),onclick:()=>App.tabState.set("result")},"result"),m("div.grid-col.tab.tab-b.ml-20",{class:App.tabState.selectedClass("explain"),onclick:()=>App.tabState.set("explain")},"explain"),m("div.grid-col.align-items-end.ml-50",m(QryInfos)),),),m("section.area-query-output",m("div",{class:App.tabState.displayClass("result")},m(QryResult)),m("div",{class:App.tabState.displayClass("explain")},m(QryExplain)),)))]),m("footer.area-footer-content.ml-auto.mr-0.text-x-small",m("div",m("a",{href:"https://github.com/a-le/"+versionInfo.appName,target:"_blank",title:"View github project page"},versionInfo.appName),m("span",m.trust(" &copy; 2025. Licensed under "),m("a",{href:"https://www.gnu.org/licenses/agpl-3.0.html",title:"View License",target:"_blank"},"AGPL"),),)));}};const ConnForm={conns:[],DBerror:"",connecting:false,oninit:()=>{m.request({method:"GET",url:"/api/config/cnxnames",}).then(function(response){ConnForm.conns=response.data;});},submitConnect:(conn)=>{ConnForm.connecting=true;m.request({method:"GET",url:"/api/connect/:conn",params:{conn:conn},credentials:"include",headers:getRequestHeaders(),extract:getRequestExtract(),}).then(function(response){ConnForm.connecting=false;if(response.DBerror!==""){ConnForm.DBerror=response.DBerror;}
else{App.conn=conn;ConnInfos.get();SchemaForm.get();DataDict.getTables();DataDict.getViews();DataDict.getProcedures();}});},saveToLocalStorage:(k,v)=>{localStorage.setItem(App.conn+":"+k,v);},getFromLocalStorage:(k)=>{return localStorage.getItem(App.conn+":"+k);},view:()=>{return!ConnForm.conns?null:[m("select",{id:"connSelect",onchange:function(e){App.conn="";App.schema="";ConnForm.DBerror="";ConnForm.connecting=false;SchemaForm.reset();ConnInfos.reset();QryForm.reset();DataDict.reset();if(e.target.value===""){return;}
ConnForm.submitConnect(e.target.value);}},[m("option",{value:""},"select connection…"),m(OptGroup,{rows:ConnForm.conns})]),ConnForm.DBerror==""?null:[m("button.ml-10",{onclick:()=>{document.querySelector("#connSelect").dispatchEvent(new Event("change"));}},"retry to connect"),m("span.text-warning.ml-10",ConnForm.DBerror)],ConnForm.connecting?m(WaitingAnimation,{text:"connecting",class:"ml-10"}):null];}}
...
return m.request({method:"GET",url:"/api/command/:conn/:schema/conn-infos",credentials:"include",headers:getRequestHeaders(),extract:getRequestExtract(),params:{conn:App.conn,schema:App.schema},}).then((result)=>{ConnInfos.response=result;if(result.DBerror)
return;let i=0;ConnInfos.hostname=!result.rows[0][i]?null:{"key":result.cols[i],"value":result.rows[0][i]};i++;ConnInfos.port=!result.rows[0][i]?null:{"key":result.cols[i],"value":result.rows[0][i]};i++;ConnInfos.database=!result.rows[0][i]?null:{"key":result.cols[i],"value":result.rows[0][i]};i++;ConnInfos.schema=!result.rows[0][i]?null:{"key":result.cols[i],"value":result.rows[0][i]};i++;ConnInfos.user=!result.rows[0][i]?null:{"key":result.cols[i],"value":result.rows[0][i]};i++;ConnInfos.version=!result.rows[0][i]?null:{"key":result.cols[i],"value":result.rows[0][i]};i++;})},view:()=>{return[!ConnInfos.response?null:[ConnInfos.response.DBerror?m("div.text-warning",ConnInfos.response.DBerror):[!ConnInfos.database?null:m("div.font-sm.mr-20",m("span",ConnInfos.database.key+": "),m("span.info",{title:ConnInfos.database.value},ConnInfos.database.value.split(/[/\\]/).pop())),!ConnInfos.schema?null:m("div.font-sm.mr-20",m("span",ConnInfos.schema.key+": "),m("span.info",ConnInfos.schema.value)),!ConnInfos.user?null:m("div.font-sm.mr-20",m("span",ConnInfos.user.key+": "),m("span.info",ConnInfos.user.value)),!ConnInfos.hostname?null:m("div.font-sm.mr-20",m("span",ConnInfos.hostname.key+": "),m("span.info",ConnInfos.hostname.value+(ConnInfos.port.value?":"+ConnInfos.port.value:""))),!ConnInfos.version?null:m("div.font-sm.mr-20.no-overflow",{style:"max-width: 220px;"},m("span",ConnInfos.version.key+": "),m("span.info",{title:ConnInfos.version.value},ConnInfos.version.value))]]];}};class Dict{constructor(){this.selected="";this.list=null;this.definition=null;this.columns=null;}
changeItem(item){if(this.selected==item)
return false;this.selected=item;this.definition=null;this.columns=null;if(item==="")
return false;return true;}}
const DataDict={tables:new Dict(),views:new Dict(),procedures:new Dict(),activity:new Dict(),tabStates:{objects:new TabState(""),tables:new TabState("columns"),views:new TabState("columns")},reset:()=>{DataDict.tables=new Dict();DataDict.views=new Dict();DataDict.procedures=new Dict();DataDict.activity=new Dict();DataDict.tabStates={objects:new TabState(""),tables:new TabState("columns"),views:new TabState("columns")};},getCommandOptions(command,args){return{method:"GET",url:"/api/command/:conn/:schema/:command",credentials:"include",headers:getRequestHeaders(),extract:getRequestExtract(),params:{conn:App.conn,schema:App.schema,command:command,args:args},};},getTables:()=>{let command="tables",args=[];let opts=DataDict.getCommandOptions(command,args);m.request(opts).then((response)=>{DataDict.tables.list=response;});},getViews:()=>{let command="views",args=[];let opts=DataDict.getCommandOptions(command,args);m.request(opts).then((response)=>{DataDict.views.list=response;});},getProcedures:()=>{let command="procedures",args=[];let opts=DataDict.getCommandOptions(command,args);m.request(opts).then((response)=>{DataDict.procedures.list=response;});},getTableInfos:(item)=>{if(!DataDict.tables.changeItem(item))
