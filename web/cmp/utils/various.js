...
class UIState {
    constructor({ def, onSet } = {}) {
        this.current = def;
        this.onSet = typeof onSet === "function" ? onSet : null;
    }
    set(name) {
        this.current = name;
        if (this.onSet) this.onSet(name);
    }
    get() {
        return this.current;
    }
    ...
    selectedClass(name) {
        return (this.current === name) ? "selected" : "";
    }
    displayClass(name) {
        return (this.current === name) ? "" : "display-none";
    }
}
...
// Returns an array of { value, label, group } objects for SelectInput
function toSelectOptions(rows, emptyName, valueCol, labelCol, groupCol) {
    const options = [];
    if (emptyName !== false) {
        options.push({
            value: "",
